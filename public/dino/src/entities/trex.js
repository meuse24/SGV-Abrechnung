...
import { DEFAULT_DIMENSIONS, FPS, IS_HIDPI } from '../config/constants.js';
...
const FIRE_INTERVAL_MIN = 2000;
const FIRE_INTERVAL_MAX = 6000;
const FIRE_DURATION_MIN = 700;
const FIRE_DURATION_MAX = 1200;
...
export class Trex {
    ...
    constructor(canvas, spritePos, resourceProvider) {
        ...
        this.fireCooldown = 0;
        this.fireDuration = 0;
        this.nextFireDelay = this.getNextFireDelay();
    ...
    draw(x, y) {
        ...
        // Apply gold color filter to the dino
        this.canvasCtx.save();
        this.canvasCtx.filter = 'sepia(100%) saturate(300%) brightness(100%) hue-rotate(0deg)';
        ...
        // Restore canvas state (remove gold filter)
        this.canvasCtx.restore();
        this.drawFire(outputWidth, outputHeight);
    }
    /**
    ...
    getNextFireDelay() {
        return FIRE_INTERVAL_MIN +
            Math.floor(Math.random() * (FIRE_INTERVAL_MAX - FIRE_INTERVAL_MIN));
    }
    getNextFireDuration() {
        return FIRE_DURATION_MIN +
            Math.floor(Math.random() * (FIRE_DURATION_MAX - FIRE_DURATION_MIN));
    }
    updateFire(deltaTime) {
        if (this.status === Status.WAITING || this.status === Status.CRASHED) {
            this.fireActive = false;
            this.fireTimer = 0;
            this.fireCooldown = 0;
            this.fireDuration = 0;
        ...
        if (this.fireCooldown >= this.nextFireDelay) {
            this.fireActive = true;
            this.fireTimer = 0;
            this.fireDuration = this.getNextFireDuration();
            this.fireCooldown = 0;
        }
    }
    drawFire(outputWidth, outputHeight) {
        if (!this.fireActive || this.status === Status.CRASHED) {
            return;
        }
        const frame = Math.floor(this.fireTimer / FIRE_FRAME_MS) % 3;
        const baseLength = [14, 18, 12][frame];
        const baseHeight = [6, 7, 5][frame];
        const pixel = Math.max(2, Math.round(outputHeight * 0.08));
        const mouthX = Math.round(this.xPos + outputWidth);
        const mouthY = Math.round(this.yPos + outputHeight * 0.26);
        this.canvasCtx.save();
        this.canvasCtx.globalAlpha = 0.9;
        for (let i = 0; i < baseLength; i += pixel) {
            const t = i / baseLength;
            const color = t < 0.35 ? '#ffb347' : t < 0.7 ? '#ff8c2a' : '#ff6b1a';
            const jitter = (frame === 1 ? -1 : 0) + (i % (pixel * 2) === 0 ? -1 : 0);
            const h = Math.max(2, baseHeight - Math.round(t * baseHeight * 0.6));
            this.canvasCtx.fillStyle = color;
            this.canvasCtx.fillRect(mouthX + i, mouthY + jitter, pixel, h);
        }
        this.canvasCtx.restore();
    }
    getCollisionBoxes() {
